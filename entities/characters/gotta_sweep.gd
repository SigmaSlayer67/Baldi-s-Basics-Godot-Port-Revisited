...
func _physics_process(delta : float) -> void:
	...
	if wait_time <= 0.0 and not sweepin_time:
		sweepin_time = true
	...
	# If Gotta Sweep has not roamed around 5 times
	if get_real_velocity().length() <= 0.1 and cool_down <= 0.0 and wanders < 5 and sweepin_time:
		wander()
	elif wanders >= 5:
		go_home()
	...
	for i : Node3D in npc_list: # shift other npcs
		if i.get(&"velocity") != null: # check that velocity exists
			...
			if i is Player:
				var player_ref := i as Player
				if not player_ref.boots:
					player_ref.velocity = setVelocity + (0.3 * i.velocity)
					player_ref.sweeping = true
					player_ref.failSafe = 1.0
			else:
...
func get_new_cooldown() -> float:
	return rng.randf_range(120.0, 180.0)
